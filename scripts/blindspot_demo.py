...
class BlindspotDemo:
    ...
    def policy_blindspot(self, profile, candidates, k=3) -> Tuple[List[str], str, Dict[str, Any]]:
        """Trained RL agent. Falls back to a 'smart proxy' if no LLM hooked up.

        The proxy ranks by:  relevance × (1 − already_known) × novelty_bonus
        i.e. relevant *and* not in the user's existing vocabulary *and* truly novel.
        This gives the demo something coherent to show even before training finishes.
        """
        meta = {"used_trained_model": False, "reasoning": ""}
        if self.llm_generate is not None:
            try:
                ...
                return surfaced, "Trained Blindspot policy (GRPO on Qwen-9B).", meta
            except Exception as e:
                meta["reasoning"] = f"(trained model failed: {e}; using analytical proxy)"

        # Pre-training placeholder: kNN-informed proxy. Uses adoption signals
        # from similar users — a realistic upper bound for what GRPO can learn
    ...
    def compare_all(self, paragraph: Optional[str] = None,
        ...
        """Two modes:
          • paragraph=...  — TF-IDF match to closest of 17 users (general public demo)
          • user_id=...    — directly use one of the 17 real users (ground-truth demo)
        """
        ...
        for name, fn in runners:
            res = fn(profile, candidates)
            if name == "Blindspot RL":
                surfaced, descr, meta = res
            else:
                surfaced, descr = res
                meta = {}
